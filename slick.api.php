 ...
 *   // For simplicity, we use string. The expected is render array.
 *   $items[] = [
 ...
 *
 *   // Prepare $variables to pass into a .tpl.php file.
 *   $variables['slick'] = $element;
 *
 *   // Render the slick at a .tpl.php file.
 *   // print render($slick);
 ...
 *   // @see \Drupal\slick\Entity\Slick::defaultSettings()
 *   $build['options'] = [
 *     'arrows'    => FALSE,
 *     'autoplay'  => TRUE,
 *     'vertical'  => TRUE,
 ...
 * @see \Drupal\slick_test\SlickSkinTest
 *
...
function hook_slick_overridable_options_info_alter(&$options) {
  // Adds vertical options to Slick field formatters, or Slick Views UI forms.
  $options['vertical'] = t('Vertical');
  $options['verticalSwiping'] = t('Vertical swiping');
}
...
function hook_slick_optionset_alter(Slick &$slick, array $settings) {
  if ($slick->id() == 'x_slick_nav') {
    ...
    // \Drupal\slick\Entity\Slick::defaultSettings().
    // Disable arrows.
